# Dercretum-Matrix（诏令矩阵）离线安装提示词

请安装与本文件同一发布包中的 Dercretum-Matrix（诏令矩阵）。规范 skill 名与调用为
...
2. 必须安装到当前用户真实 HOME 下的 `.agents/skills/court-capability-router`；该目录名是兼容 locator，加载后必须显示 `Dercretum-Matrix（诏令矩阵）` / `decretum-matrix`。`.agents` 是唯一可主动创建的工具根。
...
6. 校验 `release-manifest.json`、`VERSION`、`references/manifests/skill-identity.v1.json` 和载荷哈希；身份清单必须声明规范产品、skill 与调用为 `Dercretum-Matrix（诏令矩阵）`、`decretum-matrix`、`$decretum-matrix`。已有目标先做逐文件 SHA256 备份，再无删除覆盖；不得移动或删除目标独有文件、史馆文件或用户配置。
