...
本次发布把 npm 载体抬到 `1.0.5-beta.0`，并同步三端安装语义：

1. 首次安装仍会自动安装或复用 `zellij` 与 `squad`，安装回执继续写入对两项开源
   项目的致谢和链接。
2. 安装后的 skill 根仍不保留哈希校验脚本；只保留安装前结构性完整性检查。
3. GitHub Packages 维持同一个包名，但新增不可变版本 `1.0.5-beta.0`。
...
2. 安装器在每次同步完成后将该文件设为只读；下一次发行同步会先临时解冻、覆盖并
   重新冻结，避免文件属性阻断更新。
3. 它继续声明 `runtime_dependency: false` 与 `runtime_loading: false`；安装后的
   轻量校验仅识别其冻结状态，不运行任何 CFT 哈希校验。
4. 产品版本为 `beta1.0.4-hotfix-v1`，npm 映射为
...
本版本在 `release/beta1.0.2` 上执行用户审查后的客观整改：不删除
...
本节只记录源码候选与本地验收目标；tag、GitHub Release、npm 和 Wiki 状态必须由
后续远端回读证明。
