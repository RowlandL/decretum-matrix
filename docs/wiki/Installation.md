...
当前 GitHub 发布线为 `beta1.1.4`；GitHub Packages 的 `beta` dist-tag 是独立通道，
以远端实时回读为准。安装成功、远端包状态和活动运行身份仍以发布后回读与安装回执为准。

> 口径说明：外部发布回读存在前，当前源码 `beta1.1.4` 按未发行候选处理；本页命令与
> 目标不自动构成安装授权，安装仍需显式授权的外置安装事务与匹配回执。
