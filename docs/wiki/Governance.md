...
[Architecture](Architecture.md) 是结果导向三省六部项目框架的维护入口。开发与
审查以其中的职责链、五级复核、失败回退和结果证据要求为基准；当前版本尚未达到的
要求应明确记录为实现差异。历史设计是来源，运行时状态与 receipt 是事实证据，
二者都不能替代最新用户旨意或把未履职官署记录成已履职。
