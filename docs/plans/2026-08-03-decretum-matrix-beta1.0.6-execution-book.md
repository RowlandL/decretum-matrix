...
- 当前状态：`M4_FINAL_ACCEPTED_ALL_STAGES_CLOSED`；唯一 cursor=`stage-4/semantic-lineage`（依陛下裁决自 `stage-3/result-recovery-chain` 推进至 M2 宏阶段首子门，见 log Entry 0032；charter 裁定见 Entry 0033；相二 RED/GREEN、D6 聚焦验证、D7 刑部复查与 D8 回写见 Entry 0034/0035；门下验收见 Entry 0039（独立只读复验 E1-E8 全绿、F 停止门未触发、写集符合、R-07 转绿、R1 记载在位）；archive receipt=`shiguan:SCOSZLSZU9T-20260807-1-DCBB`（path=`C:/Users/32893/.agents/court-shiguan/decretum-matrix/references/plan-archives/plan-20260807-stage4-semantic-lineage-subgate-1.md`；closeout_identity 逐字复制：诏令编号：SCOSZLSZU9T-20260807-1-DCBB / 古制谱系：史馆总纪·朝制志·官署门·三省六部纲·政令流转目·上奏回奏条·源摘要为英文；原文保留在源字段...诏 / 作业AI：workbuddy-root-serial-inline）；**M2 三子门（语义/投影/迁移）全部闭合**（投影：Entry 0040-0043 + `SCOSZLSZU9T-20260807-3-DCBB`；迁移：Entry 0044-0047 + `SCOSZLSZU9T-20260807-4-DCBB`）；**M3 阶段 charter 裁定见 Entry 0048**（陛下授权「保留可回滚基准」裁定）；**M3 源码 acceptance RED/GREEN 闭合见 Entry 0049**（R-PA1 package_skill web/shiguan-tree 白名单 + R-PA2 portability env fail-closed，E1-E8 全绿）；**M3 候选包基线提交与构建见 Entry 0050**（HEAD 自 48ddc910 推进至 8e1d840，四笔提交未 push；候选包 `npm-build-beta106-probe/beta1.0.6/8923957e5d…/` provenance 绑定）；**M3 候选包验收门全绿见 Entry 0051**（validate_zip 177 entries/payload 0 problems/privacy 63 OK/receipt head_commit=8923957）；**全流程审查见 Entry 0052**（11 项发现：执行书滞后/F 编号笔误/-B 复发已整改）；**§4.4 install receipt 生成端补全见 Entry 0054**（R-I1 RED/GREEN：install APPLY 产出十字段 receipt 落盘 + E5 manifest 修复）；**M3 真实安装见 Entry 0055**（受控通道 write=True：四物理根 103/103 与 CHILD 逐字节一致、backup=projection-dff93e3d、receipt=install-5be70535、checker 主命令 PASS）；**真实安装门下验收 ACCEPT 见 Entry 0056**（独立复验全绿）；**M3 宏阶段闭合见 Entry 0057**（legacy 迁移零触发裁定；M4 传递项 6 项）；**M4 charter 三省六部流转分轮自审 APPROVE 见 Entry 0058**（远端只读核验本轮执行/发布另授权、version 对齐列待批、短结诏、维持钉定；语义纠正：用户=陛下，审查按三省六部流转机制分轮）；**M4 RED/GREEN 见 Entry 0059**（workspace.yaml version.current beta1.0.5→beta1.0.6 陛下批准治理变更、identity manifest 补 skill_sha256 同步四根、npm registry 404=无发布对象、R-M4-1 doctor 识别口径不修、R-M4-4 旁路）；**M4 门下终审 ACCEPT 见 Entry 0060**（capability evidence 无晋升项保持 PLANNED_UNVERIFIED、E1-E7 全绿、M1-M4 全阶段闭合）。**beta1.0.6 本地阶段全部完成**；外部发布仍须陛下另行授权。
- **发布阶段（2026-08-07 陛下授权后）：发布阻塞缺陷修复 + 全量回归重验 + npm 发布完成**（log Entry 0063/0064）。**发布阻塞缺陷一**（npm postinstall 全新环境必败）：M2 迁移子门让 sync 无 receipt 时 fail-closed，全新 HOME 必败（1.0.5 回归）→ 修复 sync 首装 canonical+receipt（commit 5b649d5）；**缺陷二**（smoke 契约漂移）：launcher 写 body_hashes:0 但 mjs 契约断言不含 → 契约对齐（commit 601de4d）。**全量回归全绿**：四根重同步（check_active_copy_hashes PASS roots=4 files=148）、候选包重建（npm-build-beta106-probe/beta1.0.6/ebe038a…/，zip sha256=04091765…）、release 五件套重建（attestation 绑定 601de4d）、npm 包构建 EXIT=0（installed_package_smoke PASS，全新隔离 HOME postinstall 成功）。**npm 发布成功**：@rowlandl/decretum-matrix@1.0.6-beta.0（registry=https://npm.pkg.github.com，dist-tag=beta，access=public），远端版本创建于 2026-08-07T06:18:55Z（package id 1107469911），gitHead=601de4d、tarball shasum=63ed47ab… 与本地一致，dist-tags {beta: 1.0.6-beta.0} 复验通过。**HEAD=601de4d（detached，三笔本地提交未 push）；tag beta1.0.6 前移至 601de4d（annotated，未 push）**。
- **外部发布全部完成（2026-08-07 陛下授权「准，然后要落盘一份 handoff 交接文档」）**：git push HEAD → origin `release/beta1.0.6`（5eed135..1423256，快进）；tag `beta1.0.6` 前移推送（force-with-lease 远端 d9edad8 误推旧 tag → 本地 4a6c961，peel `601de4d` 与 npm gitHead 一致）；GitHub Release 创建 `https://github.com/RowlandL/decretum-matrix/releases/tag/beta1.0.6`（Pre-release，targetCommitish=`601de4d`，5 项资产 zip/sha256/attestation/SBOM/release-notes，zip sha256=`04091765…`）；handoff v5 落盘 `D:/project/docs/handoffs/decretum-matrix-beta106-publish-closeout-handoff-v5-20260807.md`（发布终态锚点，Entry 0065 记账）。
- **对外文档净化 + 正式发布（2026-08-07 陛下授权「A+B 就行…尽量不重发…将首页 Switch branches/tags 默认改为 beta1.0.6，预发布改正式」）**：**A（在线）** GitHub Release notes 净化为公众客观描述；**B（源文件）** README.md / RELEASE-LOG.md / docs/wiki/Release-Notes.md 净化内部治理隐喻（尚书省/六部/史馆/结诏/太子/三省/差遣/开朝/回奏/官署 → 分层治理/记忆/归档/主协调等），保留技术标识符（court.*、shiguan、superCC 附「（高级执行）」、品牌「诏令矩阵」）；release-manifest.json 重生成（README 哈希 28ef3d6f→8fa6e5d1）+ check_release_manifest 49 步全过（R3 复跑义务）；commit `e50ed8b` push 至默认分支。**默认分支** `release/beta0.5.10` → `release/beta1.0.6`（首页 Switch branches/tags 默认值）；**Release 转正式**：`--prerelease=false` + `make_latest=true` → beta1.0.6 = **Latest + 正式**（原 Latest beta1.0.1 让位）。**不重发 npm**（已发布包不可变，净化后源文件供下一版使用）。CHILD 干净、HEAD=e50ed8b。
...
10. M2 迁移子门闭合：charter APPROVE（Entry 0044，含备份禁删约束）、RED 落笔 R-M1~R-M4 全 FAIL（Entry 0045）、GREEN 转绿 + D6/D7（Entry 0046）、门下验收 `ACCEPT`（Entry 0047，M2 三子门全闭）；archive receipt=`shiguan:SCOSZLSZU9T-20260807-4-DCBB`。
11. M3 阶段 charter 裁定（Entry 0048，太子授权「保留可回滚基准」：默认源码 acceptance+候选包构建+fixture 验证，受控安装通道保留）。
12. M3 源码 acceptance RED/GREEN 闭合（Entry 0049）：R-PA1 package_skill web/shiguan-tree 白名单修复、R-PA2 portability env fail-closed；E1-E8 聚焦验证全绿。
13. M3 候选包基线提交与构建（Entry 0050）：HEAD 自 48ddc910 推进至 8e1d840（四笔本地提交未 push）；候选包 `npm-build-beta106-probe/beta1.0.6/8923957e5d…/` provenance 绑定（zip/SBOM/receipt/release-notes）。
14. M3 候选包验收门全绿（Entry 0051）：validate_zip 177 entries / payload 0 problems / privacy 63 tests OK / receipt head_commit=8923957 一致。
15. 全流程彻底审查（Entry 0052）：11 项发现（执行书滞后/F 编号笔误/-B 复发已整改；relativeWorktrees 跨工具风险、Entry 0038 标题层级等传递项）。
16. 当前待办：真实安装（默认不做，受控通道待用户授权，可回滚基准）；真实 legacy 迁移落盘（M3 授权后）；M3 阶段闭合验收与执行书最终终更。
17. §4.4 install receipt 生成端补全（Entry 0054，R-I1 RED/GREEN）：install_current_agent_copy write=True 产出十字段 receipt 落盘 install-receipts/；E5 manifest 修复（cli-command-surface 注册 boundary/stage3 两脚本）。
18. M3 真实安装完成（Entry 0055，受控通道）：四物理根 103/103 投影与 CHILD 逐字节一致；backup=projection-dff93e3d；§4.4 receipt=install-5be70535（10 字段齐、sha256 自洽）；checker 主命令 PASS（148 文件、drift=0）；junction 根（.qoder/.workbuddy/.hermes）自动跟随；安装前快照 pre-install-snapshot.json（可回滚基准）。
19. M3 真实安装门下验收 ACCEPT（Entry 0056）：独立复验全绿；M3 核心三环（源码 acceptance/候选包/真实安装）闭合。
20. 当前待办：真实 legacy 迁移落盘裁定（真实环境无 court-capability-router 残留，预计零触发）；M4 传递项整理（R3 manifest 哈希→发布复跑义务、真实 host delivery 复验、安装版缺 shard 独立 P0、relativeWorktrees 跨工具风险）；M3 宏阶段闭合与 M4 charter 拟制（计划书 L201-208）。
21. M4 RED/GREEN 闭合（Entry 0059）：workspace.yaml version.current beta1.0.5→beta1.0.6（陛下批准治理变更，正则单行精确替换）；identity manifest 补 skill_sha256 同步四根；npm registry 404=无发布对象（远端核验关闭）；R-M4-1 doctor 对 worktree 识别口径不修；R-M4-4 子项目 release 分支旁路。
22. M4 门下终审 ACCEPT（Entry 0060）：capability evidence 无晋升项（保持 PLANNED_UNVERIFIED）；E1-E7 全绿；**M1-M4 全阶段闭合，beta1.0.6 本地阶段完成**；外部发布仍须陛下另行授权。
23. beta1.0.6 本地阶段结诏（Entry 0061）：archive-checkpoint --full-record 归档 `shiguan:SUIKUIKUIKUIKUIKU9T-20260807-8-DCBB`（plan-20260807-beta1-0-6-closeout-1.md）；十四行 full-record 在案；M1-M4 全阶段闭合终态。
24. 发布阶段完成（log Entry 0063/0064，陛下授权后）：发布阻塞缺陷修复（sync 首装 canonical+receipt `5b649d5`、smoke 契约对齐 `601de4d`）+ 全量回归重验全绿（四根重同步/候选包 ebe038a/release 五件套/npm 构建 EXIT=0 smoke PASS）+ npm 发布成功（`@rowlandl/decretum-matrix@1.0.6-beta.0`，dist-tag=beta，远端复验 gitHead=601de4d、shasum=63ed47ab 一致）。HEAD=601de4d（三笔本地提交未 push）；tag beta1.0.6 前移 601de4d（未 push）。下一动作（待陛下另行授权）：git push HEAD + tag push + GitHub Release 创建。
25. 外部发布全部完成（2026-08-07 陛下授权「准，然后要落盘一份 handoff 交接文档」）：①git push HEAD → origin `release/beta1.0.6`（5eed135..1423256，快进）；②tag `beta1.0.6` 前移推送（force-with-lease：远端 d9edad8 误推旧 tag → 本地 4a6c961，peel `601de4d`，与 npm gitHead 一致）；③GitHub Release 创建 `https://github.com/RowlandL/decretum-matrix/releases/tag/beta1.0.6`（Pre-release，targetCommitish=`601de4d`，5 项资产：zip/sha256/attestation/SBOM/release-notes，zip sha256=`04091765…`）；④handoff v5 落盘 `D:/project/docs/handoffs/decretum-matrix-beta106-publish-closeout-handoff-v5-20260807.md`（发布终态锚点）。CHILD 工作区干净、detached HEAD=1423256。遗留传递项：真实 host delivery 复验、安装版缺 shard 独立 P0、R3 复跑义务、relativeWorktrees 跨工具风险、子项目 release 分支旁路、doctor R-M4-1 不修。
26. 对外文档净化 + 正式发布（2026-08-07 陛下授权「A+B 就行…尽量不重发…将首页 Switch branches/tags 默认改为 beta1.0.6，预发布改正式」）：**A（在线）** GitHub Release beta1.0.6 notes 净化为公众客观描述；**B（源文件+本地资产）** README.md / RELEASE-LOG.md / docs/wiki/Release-Notes.md 净化内部治理隐喻（尚书省/六部/史馆/结诏/太子/三省/差遣/开朝/回奏/官署等 → 分层治理/记忆/归档/主协调等公众表述），保留技术标识符（court.*、shiguan、superCC 附「（高级执行）」说明、品牌名「诏令矩阵」）；release-manifest.json 重生成（README 哈希 28ef3d6f→8fa6e5d1）+ check_release_manifest 49 步全过（R3 复跑义务履行）；commit `e50ed8b` 已 push 至默认分支。**③默认分支**：GitHub 默认分支 `release/beta0.5.10` → `release/beta1.0.6`（首页 Switch branches/tags 默认值）；**Release 转正式**：beta1.0.6 `--prerelease=false` + API `make_latest=true` → **Latest + 正式**（原 Latest beta1.0.1 让位）。**不重发 npm**（已发布包保持不可变，净化后的 README/RELEASE-LOG 供下一版使用）。CHILD 干净、HEAD=e50ed8b。
