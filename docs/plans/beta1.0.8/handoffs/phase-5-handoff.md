...
- git_head_commit: 5e0b660（内容提交）；本 handoff 文档随后续 docs commit 提交
...
- check_active_copy_hashes extra=4（受保护史馆锚点）：契约 NO_MOVE；干净安装根复验应无 extra；如 REVIEWER 要求本机全绿，需另行授权移动史馆锚点（当前不建议）。
...
- check_install_current_agent_copy.py self-test ValueError：beta1.0.7 既有问题（文件未变更），建议后续版本修复。
