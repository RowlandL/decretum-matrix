...
### 2026-07-19 beta0.5.12 外部发布终态与 beta0.5.13 successor（append-only receipt）

- **发布终态：** `BETA0.5.12_PUBLICATION_TERMINAL=PASS`；accepted source=`release/beta0.5.12@e3fcecf4f8869a97bec1da465134f2ce7bd14f46`，tree=`eb3bbd6c4d5b0ded56069ff73fa065b4bab34aba`。annotated tag object=`f3e714ae75f4e40cfc57919dcd3c978a3ed9d793`，peeled commit 与 accepted source 相同。
- **GitHub：** branch/tag 已用 non-force atomic push 创建；prerelease=`https://github.com/RowlandL/decretum-matrix/releases/tag/beta0.5.12`，release id=`356145630`，target=`release/beta0.5.12`，五项资产 metadata digest 与 fresh download SHA-256 全部通过。release ZIP SHA-256=`2f3ba17694b147affbb5cdf300dd782776e7703c8e3dbf6d8167afc934d4f3ef`，与 accepted candidate 完全一致，未重建或重标候选。
- **npm：** `@rowlandl/decretum-matrix@0.5.12-beta.0` 已发布到 `https://npm.pkg.github.com`，visibility=`public`，package/version id=`13423315/1043616571`，`beta -> 0.5.12-beta.0`。TGZ SHA-256=`e47e2f7360d5ac4852ec03c9b57c61670ccdcce3111312e0fd706ca320a28542`；fresh online install、repository linkage 与内嵌 release ZIP hash 全部 `PASS`，临时 npmrc/smoke 目录已清除且 token 未持久化。
- **README / Wiki：** README 保留最短安装命令 `npm install @rowlandl/decretum-matrix@beta --registry=https://npm.pkg.github.com`。GitHub Wiki 已初始化并同步到 `decretum-matrix.wiki.git@783701ba1f81d3d8f7e48069cd86c4b88e75dbd1`；离线 `docs/wiki` exact set=`8`，LF 原字节 fresh clone 得到 `ONLINE_OFFLINE_CONSISTENT`，线上 HTTP=`200`。主机默认 `core.autocrlf=true` 的 CRLF checkout 只作转换前像，不冒充内容漂移。
- **终态回执：** combined publication receipt=`D:\project\release-staging\decretum-matrix\beta0.5.12\e3fcecf4f8869a97bec1da465134f2ce7bd14f46\receipts\publication-terminal-receipt.json`，SHA-256=`458d4dd05cf4d249620da70df213d76fe4da7a8419eeb2ffe39608f35c94c92d`；GitHub/npm/Wiki 子回执 SHA-256 分别为 `283e01384b8fae5ba34818baeb42dfcb9778ea61ffe71d7b18e78bafb4991ff9`、`e313e9dae9921b2bcedd0e5402b778bac02401f881fb1d7c0910fa00d806ab8f`、`dc13e15ca8d0c534ee144b72474c022a61014e780ed77f2e5dd0e6d4f0359a20`。
...
### 2026-07-19 Phase 10 protected-Shiguan no-read install repair（append-only receipt）

- **首错与停止：** accepted commit `394872b1dbdd91b0470de1d7d919065a1bd7a8ca` 的 tagless candidate 已通过 pre-install source/package gate，但安装前代码审查发现 updater 与 active-copy checker 会读取/哈希 `.agents` 四个 protected Shiguan index/knowledge-graph 锚点，违反本执行书 `NO_READ | NO_WRITE | NO_MOVE | NO_REWRITE`。因此 host install=`NOT_RUN`，五根未修改，未进入 post-install 或发布。
- **旧候选处置：** ZIP SHA-256=`91A5FC48B5A992B8FC4EC3BF2482A0537414B3E2684206EBCE14C8FCCAAB8FC1` 保持原目录、原字节，状态=`SUPERSEDED_UNPUBLISHED`；不得安装、promotion、tag、上传或删除，不得用它冒充最终 beta0.5.13 candidate。
- **有界修复：** 本段随唯一 Phase 10 install-privacy repair commit 生效，accepted commit binding=`THIS_COMMIT`。统一 updater 只处理 manifest 管理的公开 projection；Shared Shiguan protected path operation count 固定为 `0`，既有数据不读取、不比较、不创建、不覆盖。非共享工具根出现已知 protected path 时只用存在性 metadata 拒绝第二份数据副本；active-copy checker 改用 projection-managed public inventory，禁止 whole-root recursive scan。公开 managed files 的逐文件 backup、失败自动 rollback 与显式 rollback 合同保持不变。
- **focused GREEN：** installer cases=`28/28 PASS`、configuration cases=`31/31 PASS`、`protected_shiguan_data_accessed=false`；五根 `SKILL.md` read-only probe=`PASS`，active-copy receipt=`protected_shiguan_data_access=NOT_RUN_NO_READ_POLICY`、operation count=`0`。
- **证据：** `D:\project\.repo-control\evidence\decretum-matrix\CCR-R2-SHIR-20260714-A02\phase10\phase10-shiguan-no-read-install-repair-20260719.json`，SHA-256=`A4C4038AD04927B3240794143CAEF6B1A217B7E74B54545743804DFBFC2115E0`；rollback preimage 位于 `.repo-control/preimages/decretum-matrix/CCR-R2-SHIR-20260714-A02/20260719T094500-phase10-shiguan-no-read-install`。
- **重新准入顺位：** `SOURCE_GATES -> BOUNDED_REPAIR_COMMIT -> NEW_TAGLESS_CANDIDATE -> CANDIDATE_REUSE_GATE -> PRE_INSTALL_EXACT_ZIP -> FIVE_ROOT_BACKUP/APPLY/VERIFY -> POST_INSTALL_EXACT_ZIP`。提交前必须重生 release manifest 并通过全量 source/package/privacy/SPEC/QUALITY、`git diff --check`、index=`0`、`.pyc=0`；否则本段状态保持 `BLOCKED_BEFORE_COMMIT`。

### 2026-07-19 Phase 10 release-manifest projection repair（append-only receipt）

- **native reread 首错：** `cb92c1bc576f249794751c82f6ede00d13db2a5f` candidate ZIP SHA-256=`04882B06535FB0C54F621622EE2FBF61A49A5DCB2BE56175A8200277341BABFE` 已完成 backup/apply、14-role bounded sync 与 post-install `4/4 PASS`；但五根 native reread 发现 `VERSION/identity=beta0.5.13` 时 `release-manifest.json` 仍为 beta0.5.10，证明 install projection allowlist 漏装当前 release manifest。该候选状态改为 `SUPERSEDED_UNPUBLISHED`，不得发布或复用。
- **强制回滚：** Codex 14 profile 已从外置 backup 原子恢复；skill projection 从 backup manifest SHA-256=`295B8E8DE9B0EAF763741082107FE085FBAA83B234A5F97127D02F7A8368807D` 恢复 `368` 个 managed 操作。五根恢复为 beta0.5.12、skill SHA-256=`DD42B8CC9D3720E251FF197B3B27E69647BAD5B9009781D3C777E77C07079226`；protected Shiguan operation=`0`，backup 原样保留。
- **有界修复：** 本段随唯一 release-manifest projection repair commit 生效，accepted commit binding=`THIS_COMMIT`。`release-manifest.json` 仅加入 `shared_agents` 与 `portable_current_tool` 两个 managed projection；fixture/assertion 强制回读该文件。未加入 SBOM、repository-only docs 或其他扩大面，protected Shiguan no-read/no-write 合同不变。
...
### 2026-07-19 Phase 10 physical-root migration and one-shot install overlay

- **最新明确安装旨意：** 本 overlay 仅 supersede Phase 5.1/5.2 中“npm 安装不得运行 lifecycle、空白机只保留显式二次安装入口”的旧条款。正式 `beta` 包必须继续保留精确命令 `npm install @rowlandl/decretum-matrix@beta --registry=https://npm.pkg.github.com`，并由受限 `postinstall` 在同一次安装内调用 Phase 5.1 唯一 updater core；不得复制第二套安装状态机。
- **覆盖与回滚：** 安装只覆盖 projection manifest 管理的公开代码/控制面文件。覆盖前必须创建外置逐文件 SHA-256 preimage 和可消费 rollback receipt；旧 `skills/court-capability-router` 物理目录的成功 rename/move 也必须支持安装后显式回滚，不得再限定为事务内自动回滚。目标独有文件与史馆用户数据不进入覆盖集。
- **史馆物理根：** canonical 权威路径固定为 `%USERPROFILE%\.agents\court-shiguan\decretum-matrix\references` 的真实物理目录，symlink、junction 或 generic reparse 作为 canonical 一律 fail closed。若旧路径仍是物理目录，迁移脚本只允许同卷原子 `os.replace`，以目录 identity 证明资料未复制、未逐正文读取；若 canonical 与旧根同时为物理目录则立即阻断。旧 locator 只能在物理迁移完成后创建精确 Windows junction 兼容定位，不能以链接代替资料迁移，也不能形成第二物理 authority。
- **空白机一次到位：** 默认 npm lifecycle 依次执行 `embedded ZIP/sidecar verify -> canonical public projection backup/apply -> physical Shiguan topology create/migrate -> seed transaction -> Obsidian registration -> service task install -> receipt`。npm 阶段不强制即时启动 daemon；缺少 Python 3、hash/identity 不匹配、双物理根、canonical link、备份冲突或任一事务失败均 fail closed 并执行有界 rollback。显式 `--ignore-scripts` 仍可禁用宿主写入，但不得冒充完成安装。
- **本机最终收敛：** Phase 10 五根最终安装仍通过同一 updater 显式授权 fanout；除史馆用户数据、pending/private 正文与未授权配置外，诏令矩阵、三省六部 profiles/dossiers/commands/scripts、Obsidian 控制面、service/autosync/web 入口均升级到 accepted beta0.5.13 projection。史馆用户数据保持原字节与原位置，不以再次复制冒充迁移。
- **边界与性能：** `pending_body_access=NO`；对 pending/private body 的 read/hash/move/delete/mark count 均为 `0`。修复期保持严格串行；只有串行 source/install gates 全部 PASS 后才允许一次小规模并行加载 smoke。性能门继续使用 Phase 5.2 已批准目标：fast-open 相对 legacy 至少提升 `30%`，并复用既有 cold/warm p50=`158.390/9.065 ms` 作为不回退基线。
- **提交与后续游标：** 本段随 Phase 10 physical-root/one-shot-install repair 的唯一有界提交生效，accepted commit binding=`THIS_COMMIT`。顺位固定为 `FOCUSED_GREEN -> FULL_SOURCE_GATES -> REPAIR_COMMIT -> NEW_TAGLESS_CANDIDATE -> FIVE_ROOT_BACKUP/APPLY/VERIFY -> CONTROL_PLANE_CONVERGENCE -> POST_INSTALL/PREPUBLICATION -> beta0.5.13 PUBLICATION -> SAME_WORKTREE beta1.0.0 HANDOFF`；不得复用已标记 superseded 的旧 candidate。

### 2026-07-19 beta0.5.13 发布终态与同 worktree beta1.0.0 自动交接（append-only receipt）

- **A02 accepted source：** `release/beta0.5.13@907301cfc308debfe5d64ce3951fe96ee342b2be`，tree=`43d25f1ffbc2364bb2592efcc1426b1956e4718a`；annotated tag=`beta0.5.13`，tag object=`1114f46d2577bb12ad7ac8b567ab69a268229cea`，peeled commit 与 accepted source 相同。已发布 branch/tag 均未 force，未改 main、PR、default branch 或根仓。
- **候选、安装与发布等式：** candidate/pre-install/install/post-install/GitHub final ZIP SHA-256 均为 `F0D02134ABB011F2C0DCC28FEFBCC387EB5653604C16691E4DAC07086310B647`；final promotion=`PASS`。GitHub prerelease=`https://github.com/RowlandL/decretum-matrix/releases/tag/beta0.5.13`，5 个资产 API digest 与 fresh download hash 全部一致。
- **npm 与空白机：** `@rowlandl/decretum-matrix@0.5.13-beta.0` 已发布到 `https://npm.pkg.github.com`，`beta -> 0.5.13-beta.0`，visibility=`public`，package/version id=`13423315/1044827740`，TGZ SHA-256=`02695BA31D99D150A396CFD68F6705E1996F0CB4F7D114A3E10F834F43E0E19A`。隔离 HOME 的真实在线命令 `npm install @rowlandl/decretum-matrix@beta --registry=https://npm.pkg.github.com` 一次完成 physical canonical runtime、physical Shiguan root、备份与 receipt；embedded ZIP hash=`PASS`，正文 read/hash=`0`，临时 npmrc/fixture 已清除且 token 未持久化。
- **五根与史馆：** 五个 canonical locator 的 `252` 个 managed public files 无漂移，`VERSION=beta0.5.13`、SKILL SHA-256=`AC3E8C9C3540B612A69BE174DF5D51D6297CF3440E57ED9A660F24C936289106`、release-manifest SHA-256=`1ADEEE33B2BD3732448C5A899F76E5F5305F6B81C6D73939493C138CB09354C3`。史馆唯一物理 authority=`C:\Users\32893\.agents\court-shiguan\decretum-matrix\references`；两个旧 locator 均为精确 Windows junction。WebUI 已从旧 Hermes loaded process 有界切换到 `.agents` 权威代码根，public/private health=`PASS`，watchdog=`ok=true`，autosync=`HEALTHY/idle`；pending body read/hash/move/delete/mark count=`0`。
- **superCC 边界：** 当前宿主不在对应 superCC 运行环境，runtime gate=`NOT_APPLICABLE_ENVIRONMENT_ABSENT`；但 superCC 与 Codex 共用的 `agents/standing-officials` 权威配置已通过 `14/14` profile、truth-gate 与 Codex projection 同一性核验，不建立第二官署配置面。
- **Wiki 与终态回执：** GitHub Wiki=`master@583db4aa5f76ec5e7c4367adad41ed3ace8a0e17`，8 页 fresh clone=`ONLINE_OFFLINE_CONSISTENT`，HTTP=`200`。publication terminal receipt=`D:\project\release-staging\decretum-matrix\beta0.5.13\907301cfc308debfe5d64ce3951fe96ee342b2be\receipts\publication-terminal-receipt.json`，SHA-256=`3EE3D687E3DF36EDC92F0E6977A07010F2B7A0A73762B8863D4D9359463296FC`；GitHub/npm/Wiki/runtime 子回执 SHA-256 分别为 `0E43633005F03F1FEA28595DFA88F7C9BEC96311A68517606A97D0855DE7E704`、`C4049EE5C57DB65A01C0D0E43DD9BAC1B0AF375B05BEEA8D90F3BCFDB2A0B30A`、`1DE053E1E456D3C915A3472F3863D6E138977C1EA699A8FD83703C258726EF2C`、`56710114623222A933AFFD6580EB0648370824ACC978EBC960D99603841301C3`。
- **自动交接：** 当前唯一 child worktree 原地从 accepted commit 创建并切换到 `release/beta1.0.0`；未创建/物化第二 worktree，`release/beta0.5.14` 不存在。A02 writer 终态后由同一受控载体取得最终阶段唯一 write lease；交接游标=`SHENG_WU_AI_LE_FINAL_STAGE_READY -> BETA1_VERSION_ALIGNMENT_GATE`。beta1.0.0 的 push/tag/GitHub release/npm publish 仍未授权且不得提前执行。
- **边界：** root/child index=`0/0`、root 既有 dirty 原样保留、`.pyc=0`、`pending_body_access=NO`、`OFFICE_PACK_Q1_Q8=DEFERRED_TO_QUEUED_POST_A02_HANDOFF`。A02 completion contract 与 Phase 10=`PASS`；后续主线只剩《声无哀乐》最终 major stage，`NEXT_MAJOR_STAGE=NONE`。
