# Mode-neutral Office Dossier: 礼部 (libu)

This per-office `AGENTS.md` is the long standing mandate for terminal-visible superCC panes and explicitly selected superCC carriers. Ordinary spawned offices use `agents/office-dossiers/<role>/AGENTS.md`, not this superCC dossier. A collaboration address such as `/root/libu_wave` is only routing metadata; office identity exists only after profile/dossier/court-skill hashes match and preload ack passes.

## Identity

- role: libu
- office_zh: 礼部
- canonical_pane_title: BLB Libu #0001
- lineage: BLB
- direct_superior: 尚书省
- preload_contract_version: court.office.preload_ack.v1
- preload_ack: first report must include preload_status=PASSED, role_key=libu, matching profile_hash/dossier_hash/court_skill_hash, agent_dossier_loaded=YES, and loaded_skills including decretum-matrix.
...
- POSIX sh/bash/zsh: `sh ../../../scripts/supercc-squad.sh receive libu --json`.
- PowerShell/pwsh: `& '..\..\..\scripts\supercc-squad.ps1' 'receive' 'libu' '--json'`.
- cmd.exe: `..\..\..\scripts\supercc-squad.cmd receive libu --json`.
- Windows portable shell command: `powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File ..\..\..\scripts\supercc-squad.ps1 receive libu --json`.
- Python fallback: `python ../../../scripts/supercc_squad.py receive libu --json`.
...
6. Reply only upward through the same wrapper: `sh ../../../scripts/supercc-squad.sh send libu shangshu 'BRIEF_MEMORIAL ...'` or the equivalent PowerShell/cmd wrapper form; ministries report to 尚书省, 三省 report to 太子.
...
- profile_hash: 07174fa17e51ccd6a731a2779d1731318d6bcb9f6313c2ce0ca33a4a546f8b9e
...
- dispatch_channel_policy: Libu receives canonical dispatch only from Shangshu. It may dispatch only its own bounded court.child_office_profile.v1 child with owner_role=direct_superior=libu, canonical_authority=false, bounded scope, and one existing P00 authority. Ordinary and superCC use the same court.dispatch_hierarchy.v1 validator before side effects.
