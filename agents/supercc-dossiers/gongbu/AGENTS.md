# Mode-neutral Office Dossier: 工部 (gongbu)

This per-office `AGENTS.md` is the long standing mandate for terminal-visible superCC panes and explicitly selected superCC carriers. Ordinary spawned offices use `agents/office-dossiers/<role>/AGENTS.md`, not this superCC dossier. A collaboration address such as `/root/gongbu_wave` is only routing metadata; office identity exists only after profile/dossier/court-skill hashes match and preload ack passes.

## Identity

- role: gongbu
- office_zh: 工部
- canonical_pane_title: BGB Gongbu #0001
- lineage: BGB
- direct_superior: 尚书省
- preload_contract_version: court.office.preload_ack.v1
- preload_ack: first report must include preload_status=PASSED, role_key=gongbu, matching profile_hash/dossier_hash/court_skill_hash, agent_dossier_loaded=YES, and loaded_skills including decretum-matrix.
...
- POSIX sh/bash/zsh: `sh ../../../scripts/supercc-squad.sh receive gongbu --json`.
- PowerShell/pwsh: `& '..\..\..\scripts\supercc-squad.ps1' 'receive' 'gongbu' '--json'`.
- cmd.exe: `..\..\..\scripts\supercc-squad.cmd receive gongbu --json`.
- Windows portable shell command: `powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File ..\..\..\scripts\supercc-squad.ps1 receive gongbu --json`.
- Python fallback: `python ../../../scripts/supercc_squad.py receive gongbu --json`.
...
6. Reply only upward through the same wrapper: `sh ../../../scripts/supercc-squad.sh send gongbu shangshu 'BRIEF_MEMORIAL ...'` or the equivalent PowerShell/cmd wrapper form; ministries report to 尚书省, 三省 report to 太子.
...
- profile_hash: 8f4d747d799d76850e6e55d1ce71f0014d52a18ac4e000c5f622e4cfc3314272
...
- dispatch_channel_policy: Gongbu receives canonical dispatch only from Shangshu. It may dispatch only its own bounded court.child_office_profile.v1 child with owner_role=direct_superior=gongbu, canonical_authority=false, bounded scope, and one existing P00 authority; GongBu-GongJiang keeps role_key=gongbu. Ordinary and superCC use the same court.dispatch_hierarchy.v1 validator before side effects.
