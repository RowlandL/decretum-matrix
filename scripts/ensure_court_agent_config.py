...
This script updates the host Codex `config.toml` so portable installs use the
approved bounded recursion ceiling. These values are host limits, not an order
to fill every slot. A valid configured V1 or V2 mode is preserved by the
default `--protocol auto` check. Blank or unresolved installs still default to
V2. V1 keeps the complete V2 rollback table present but disabled:
...
- V1 additionally uses `[features].multi_agent = true` and
  `[agents].max_threads = 15`, while the V2 table remains present with
  `enabled = false`

Multi-Agent V2 counts the root inside its per-session concurrency ceiling, so
16 total slots produce at most 15 concurrently open child-agent threads.

Default mode is check-only and exits successfully even when the current config
is below the recommended target. Pass `--check` for explicit check-only mode or
`--write` to edit the config. When a running Codex Desktop process keeps an
older user-config snapshot and rewrites `config.toml`, use `--managed-overlay`
to write the protocol-only `managed_config.toml` layer instead; it preserves the
Desktop-owned user file and takes precedence for newly loaded configurations.
"""
...
def read_config_settings(text: str) -> dict[str, object]:
    ...
    if isinstance(multi_agent, dict):
        ...
        result["inactive_v2_config_preserved"] = bool(
            ...
            and v2_threads == DEFAULT_MAX_THREADS
...
def main() -> int:
    ...
    updated = desired_text(original, args.max_depth, args.max_threads, protocol=args.protocol)
    ...
    if not args.write:
        ...
        print(
            ...
            f"target_max_threads={args.max_threads} target_protocol={target_protocol}"
        )
