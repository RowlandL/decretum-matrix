...
def bootstrap_artifact(task: Mapping[str, Any]) -> dict[str, Any]:
    ...
    value = {'schema': 'court.case_bootstrap.v1', 'plan_status': 'BOOTSTRAP_UNPLANNED',
             'task_id': task['task_id'], 'charter_revision': task['charter_revision'],
...
def validate_document(value: object) -> dict[str, Any]:
    from court_dispatch_policy import OFFICE_SPECS
    ...
    for step in steps:
        ...
        if key in ids or step['role'] not in OFFICE_SPECS:
...
def _producer(task: Mapping[str, Any], role: str, value: object, events: list[dict[str, Any]]) -> dict[str, str]:
    ...
    if isinstance(task.get('case_binding'), dict):
        ...
        if (not receipt_id or not identity
                ...
                or stored.get('receipt_sha256') != agent.get('native_host_action_receipt_sha256')):
...
def current_plan(task: Mapping[str, Any]) -> dict[str, Any] | None:
    ...
    if (plan.get('sha256') != digest(body) or plan.get('task_id') != task.get('task_id')
            or plan.get('charter_revision') != task.get('charter_revision')
            or plan.get('charter_sha256') != task.get('charter_sha256')):
        raise ValueError('case_plan_artifact_stale_or_changed')
    if isinstance(task.get('case_binding'), dict):
        from court_case_binding import case_identity_sha256
        if plan.get('case_identity_sha256') != case_identity_sha256(task):
            raise ValueError('case_plan_identity_mismatch')
...
def submit_plan(task: Mapping[str, Any], document: object, producer: object, events: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if old is not None:
        if not isinstance(old, dict) or old.get('task_id') != task.get('task_id') or old.get('sha256') != digest({k: v for k, v in old.items() if k != 'sha256'}):
            raise ValueError('case_plan_artifact_stale_or_changed')
    if old and old['charter_revision'] == task['charter_revision'] and old['charter_sha256'] == task['charter_sha256'] and old['document'] == normalized and old['producer'] == author:
    ...
    artifact = {'schema': PLAN_SCHEMA, 'task_id': task['task_id'],
                ...
                'plan_id': 'PLAN-' + digest(str(task['task_id']))[:16] + '-r' + str(revision),
                'revision': revision, 'document': normalized, 'producer': author}
    if isinstance(task.get('case_binding'), dict):
        from court_case_binding import case_identity_sha256
        artifact['case_identity_sha256'] = case_identity_sha256(task)
...
def record_review(task: Mapping[str, Any], role: str, decision: str, plan_sha256: str,
    ...
    if plan is None or plan['sha256'] != plan_sha256:
    ...
    review = {'schema': 'court.plan_review.v1', 'role': role, 'decision': decision,
              'task_id': task['task_id'], 'charter_revision': task['charter_revision'],
              'plan_sha256': plan_sha256, 'producer': author}
    if isinstance(task.get('case_binding'), dict):
        from court_case_binding import case_identity_sha256
        review['case_identity_sha256'] = case_identity_sha256(task)
...
def require_reviewed_plan(task: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for role, expected in (('menxia', 'approved'), ('shangshu', 'dispatchable')):
        ...
        body = {key: value for key, value in review.items() if key != 'sha256'}
        if (review.get('sha256') != digest(body) or review.get('role') != role
                or review.get('decision') != expected or review.get('plan_sha256') != plan['sha256']
        ...
        if isinstance(task.get('case_binding'), dict) and review.get('case_identity_sha256') != plan.get('case_identity_sha256'):
            raise ValueError('case_plan_review_identity_mismatch')
