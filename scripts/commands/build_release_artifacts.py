...
def run_self_tests(root: Path = ROOT) -> dict[str, bool]:
    tests: dict[str, bool] = {
        ...
        "canonical_current_artifact_required": (
            release_payload_manifest.RELEASE_LABEL == "beta1.1.4"
            and release_payload_manifest.ARTIFACT_NAME
            == "decretum-matrix-beta1.1.4.zip"
        ),
    ...
    with tempfile.TemporaryDirectory(prefix="decretum-release-builder-self-test-") as tmp_text:
        ...
        try:
            untracked_identity = collect_candidate_source_identity("beta1.1.4", tracked_fixture)
        ...
        try:
            collect_candidate_source_identity("beta1.1.4", tracked_fixture)
        except ArtifactBuildError:
